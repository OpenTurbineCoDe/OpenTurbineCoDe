...
import openturbinecode.utils.utilities as ut    #Added by TG on 7/5 to use config.json for file addresses

config = ut.read_config()    #Added by TG on 7/5 to use config.json for file addresses
...
#        self.BB3DExe = "/home/kz/Desktop/BB3D/make_blade" #TODO: move this to the config.json file    #Commented out by TG 7/5
        self.BB3DExe = config["mefi"]["path_to_BB3D"]    #Added by TG 7/5
#        self.salomeExe = "/home/kz/SALOME-9.7.0-UB18.04-SRC/salome"
        self.salomeExe = config["hifi"]["path_to_Salome"]    #Added by TG 7/5
...
#            self.AFlist = [ str(int(np.round(np.interp(r,r_af,i_af)))) for r in r_nodes ]    #Commented out by TG 7/14
            self.AFlist = [ str(int(np.ceil(np.interp(r,r_af,i_af)))) for r in r_nodes ]    #Added by TG 7/14
            self.AFlist[0] = str(1)    #Inelegant solution added by TG 7/14. 
            #Not sure whether this is the correct arrangement of airfoil types, but at least it creates a valid AeroDyn blade file now.    TG
...
#        os.chdir(self.path_to_root+os.sep+'openturbinecode'+os.sep+'geometry'+os.sep+'lib_airfoils')    #TG 7/18 If you want to work in directory of blade files.
...
#        subprocess.Popen([self.BB3DExe, workingFolder+"/blade.dat"])
        subprocess.Popen([str(self.BB3DExe), str(bl)], shell = True)    #TG 7/5 not sure what the above line is supposed to do, but this one might be better.
    ...
    def Geo_runSalome(self, lineEdit_4, lineEdit_2, radioButton):
        ...
        for line in f1:
            for check, rep in zip(checkWords, repWords):
                ...
                #Replaces '???' in script.py with the iges file address, and replaces '!!!' in script.py with the working folder address.    TG 7/19
            f2.write(line)
        f1.close()
        f2.close()
