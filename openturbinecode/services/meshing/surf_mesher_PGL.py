...
def generateSurfMesh(R0, R, path_to_case, planform_file, airfoil_list, blend_var, output_name):
    ...
    mesher.planform_filename = path_to_case + os.sep + 'PGL' + os.sep + planform_file
    ...
    mesher.redistribute_flag = True
    # number of points on blunt TE
    mesher.chord_nte = 9  # 17
    ...
    mesher.blend_var = blend_var

    af_list = []
    for af in airfoil_list:
        af_list.append(path_to_case + os.sep + 'PGL' + os.sep + af + '.dat')

    mesher.base_airfoils_path = af_list
    mesher.surface_spline = 'cubic'
    ...
    # m.ds_root_end = 0.005
    mesher.root_type = 'cap'
    mesher.ni_root = 8
    mesher.s_root_start = 0.0
    mesher.s_root_end = 0.03
    mesher.ds_root_start = 0.001
    mesher.ds_root_end = 0.004
    mesher.cap_cap_radius = .001
    mesher.cap_Fcap = 0.9
    mesher.cap_Fblend = 0.7
    ...
    mesher.ni_tip = 11
    mesher.s_tip_start = 0.99
    mesher.s_tip = 0.995
    mesher.ds_tip_start = 0.001
    mesher.ds_tip = 0.00005

    mesher.tip_fLE1 = .5  # Leading edge connector control in spanwise direction.
    # pointy tip 0 <= fLE1 => 1 square tip.
    mesher.tip_fLE2 = .5  # Leading edge connector control in chordwise direction.
    # pointy tip 0 <= fLE1 => 1 square tip.
    mesher.tip_fTE1 = .5  # Trailing edge connector control in spanwise direction.
    # pointy tip 0 <= fLE1 => 1 square tip.
    mesher.tip_fTE2 = .5  # Trailing edge connector control in chordwise direction.
    # pointy tip 0 <= fLE1 => 1 square tip.
    mesher.tip_fM1 = 1.   # Control of connector from mid-surface to tip.
    # straight line 0 <= fM1 => orthogonal to starting point.
    mesher.tip_fM2 = 1.   # Control of connector from mid-surface to tip.
    # straight line 0 <= fM2 => orthogonal to end point.
    mesher.tip_fM3 = .2   # Controls thickness of tip.
    # 'Zero thickness 0 <= fM3 => 1 same thick as tip airfoil.

    mesher.tip_dist_cLE = 0.0001  # Cell size at tip leading edge starting point.
    mesher.tip_dist_cTE = 0.0001  # Cell size at tip trailing edge starting point.
    mesher.tip_dist_tip = 0.00025  # Cell size of LE and TE connectors at tip.
    mesher.tip_dist_mid0 = 0.00025  # Cell size of mid chord connector start.
    mesher.tip_dist_mid1 = 0.00004  # Cell size of mid chord connector at tip.

    mesher.tip_c0_angle = 40.  # Angle of connector from mid chord to LE/TE
    ...
    mesher.domain.scale(R-R0)
    mesher.domain.translate_z(R0)

    # split blocks to cubes of size 33^3
    mesher.domain.split_blocks(33)

    # Write EllipSys3D ready surface mesh
    # write_x2d(m.domain)
    # Write Plot3D surface mesh (in real size, not PGL normalization)
    mesher.domain.write_plot3d(output_folder+os.sep+output_name+".xyz")

    # Convert file so that we can use it with PyHyp
    conv.convert_fXYZ_to_uXYZ(output_folder+os.sep+output_name +
                              ".xyz", output_folder+os.sep+output_name+".unf.xyz")
...
def writePGLinputs(turbine_data, path_to_case, planform_file):
    ...
    for af in turbine_data["airfoils"]:
        ...
        with open(output_folder + os.sep + af["name"] + '.dat', 'wt') as file:
            ...
            if xs[0] == xs[-1] and ys[0] == ys[-1]:
                print("CAUTION: airfoil coordinates of " +
                      af["name"] + "formed a closed path (TE thickness=0). I will create some thickness for you...")
                xs = xs[1:-2]
                ys = ys[1:-2]
    ...
    r_coords = np.array(turbine_data["components"]["blade"]
                        ["outer_shape_bem"]["chord"]["grid"]) * (R-R0) + R0
    ze = np.zeros(np.size(r_coords))

    twist = np.array(turbine_data["components"]["blade"]
                     ["outer_shape_bem"]["twist"]["values"]) * 180. / np.pi
    ...
    r_af = np.array(turbine_data["components"]["blade"]
    ...
    thickness = np.interp(np.array(
        turbine_data["components"]["blade"]["outer_shape_bem"]["chord"]["grid"]), r_af, th_af)

    fname = output_folder + os.sep + planform_file

    # TODO: check that all rows they are the same size
    data = np.asarray([ze, ze, r_coords, ze, ze, twist,
                      chord, thickness, pitch_ax, ze])
...
if __name__ == "__main__":
    ...
    path_to_case = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(
        __file__)))) + os.sep + 'models' + os.sep + 'DTU_10MW' + os.sep + 'Madsen2019'
    ...
    # Call the function:
    generateSurfMesh(R0, R, path_to_case, planform_file,
                     airfoil_list, blend_var, output_name)
