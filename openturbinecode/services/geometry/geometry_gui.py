...
form_class = uic.loadUiType(os.path.dirname(
    os.path.realpath(__file__)) + os.sep + "geom.ui")[0]  # Load the UI
...
class Mapper(QtWidgets.QMainWindow, form_class):
    def __init__(self, myGeom: geom, parent=None, withMasterGUI=False):
        ...
        self.Geo_toolButton1.setToolTip(
            'Click here to select AeroDyn blade file')
        ...
        self.Geo_lineEdit_5.setText(
            str(self.myGeom.spar[0])+" "+str(self.myGeom.spar[1]))
    ...
    def writeToUI(self):
        ...
        if self.myGeom.turb_data:
            ...
            # clear the items in comboBox, otherwise the items adds up everywrite it executes
            self.Geo_comboBox_2.clear()
    ...
    #    self.myGeom.Geo_loadPredefinedTurbine(self.Geo_comboBox_4, self.Geo_LineEdit1,
    #                                          self.Geo_toolButton1, self.Geo_widget)

    def load_geometry(self):
        self.read_from_ui()
        self.myGeom.loadGeom(self.myGeom.AeroDynFileName,
                             self.Geo_Table1, QtWidgets, self.Geo_comboBox_2)
        self.writeToUI()
    ...
    def geo_set_grey(self):
        self.myGeom.Geo_setGrey(self.Geo_toolButton, self.Geo_comboBox_3,
                                self.Geo_lineEdit, self.Geo_comboBox_2, self.Geo_pushButton_4)

    # TODO: underlying codes for saving airfoil shapes
    def geo_load_airfoil_coords(self):
        self.myGeom.Geo_loadAFCoord(
            self.Geo_comboBox_2, self.Geo_comboBox_3, self.Geo_lineEdit)

    def geo_load_external_airfoil(self):
        self.myGeom.Geo_loadExternalAF(
            self.Geo_toolButton, self.Geo_lineEdit, QtWidgets, self.Geo_comboBox_2)
    ...
    def caller_Geo_generateGeom(self):
        self.read_from_ui()
        self.myGeom.Geo_generateGeom(self.Geo_comboBox, self.Geo_Table1, self.Geo_Table2,
                                     self.Geo_lineEdit_3, self.Geo_lineEdit_4, self.Geo_lineEdit_2,
                                     self.Geo_radioButton)

    def caller_Geo_setLofts(self):
        self.myGeom.Geo_setLofts(
            self.Geo_lineEdit_3, self.Geo_Table2, QtWidgets)
    ...
    def caller_Geo_runSalome(self):
        self.myGeom.Geo_setSalome(
            self.Geo_lineEdit_4, self.Geo_lineEdit_2, self.Geo_radioButton)
