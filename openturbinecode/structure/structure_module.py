...
class Structural:
    ...
    def setDefaultValues(self):
        ...
        if self.turb_data and self.models:
            ...
            path_to_root = os.path.dirname(os.path.dirname(
                os.path.dirname(os.path.realpath(__file__))))
            path_to_TMP = path_to_root + os.sep + "models" + \
    ...
    def RunModelUpdate_Beamdyn(self):
        ...
        PrimaryPath = os.path.join(os.path.split(self.workingmodel)[
                                   0], BeamFile["InputFile"].strip('"'))
        ...
        #
        PrimaryFile["MemberGeom"][:, 3] = PrimaryFile["MemberGeom"][:,
                                                                    3] * float(self.TwstSclFCV)
        # Write out
        BeamFile.write()
        PrimaryFile.write()
    ...
    def postprocessBeamDyn(self):
        Beamout = FASTOutputFile(os.path.splitext(
            self.workingmodel)[0]+".out").toDataFrame()
    ...
    def sendToHPC(self, lineEdit, lineEdit2, lineEdit3):
        subprocess.run(["scp", "-r", lineEdit.text(), lineEdit2.text() +
                       "@"+lineEdit2.text()+":"+lineEdit3.text()])
