...
from openturbinecode.solvers.aerodynamics.aerodyn.options import AeroDynConfig
...
def generate_aerodyn_config(location: Path, config: AeroDynConfig):
    ...
    contents = write_file_header(contents, config)
    ...
    contents = write_node_output(contents, config)
...
def write_file_header(contents, config: AeroDynConfig):
    # Header
