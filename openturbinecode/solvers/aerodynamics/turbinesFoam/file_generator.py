...
from openturbinecode.solvers.aerodynamics.turbinesFoam.options import (
    BlockMesh,
    topoDict,
    fvOptions,
    HexMeshDict,
...
class FileGenerator:
    ...
    def generate_files(self, file_path: Path):
        ...
        self.generate_decomposeParDict(file_path)

    def generate_decomposeParDict(self, file_path: Path):
        contents = f"""/*--------------------------------*- C++ -*----------------------------------*\\
/*--------------------------------*- C++ -*----------------------------------*\
| =========                 |                                                 |
...
numberOfSubdomains {self.control.num_subdomains};
...
"""
        ...
        with open(file_path / "system" / "decomposeParDict", "w") as file:
            file.write(contents)
...
deltaT          {self.control.delta_t:0.3f};
