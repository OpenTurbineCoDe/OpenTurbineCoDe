...
try:
    from rosco import controller as ROSCO_controller
...
class ControlMDAOom(om.ExternalCodeComp):
    def setup(self):
        ...
        self.WorkFast_file = self.path_to_root + \
            "/controls/DTU10MWAero15/DTU_10MW_NAUTILUS_GoM_A15_DLC1.2_Baseline.fst"
        self.WorkFast_fileloc = self.path_to_root + \
            "/controls/DTU10MWAero15/DTU_10MW_NAUTILUS_GoM_A15_DLC1.2_Baseline.fst"
        # providing these is optional; the component will verify that any input
        # files exist before execution and that the output files exist after.
        self.options['external_input_files'] = [self.yamlfile]

        # need to be defined: python wrapper is good
        self.options['command'] = ["openfast", self.WorkFast_fileloc]

    def setup_partials(self):
        # this external code does not provide derivatives, use finite difference
        self.declare_partials(of='*', wrt='*', method='fd',
                              form='forward', step=0.005)
    ...
    def compute(self, inputs, outputs):
        ...
        TurbineMorphs = TurbineMorph(
            yamlfile=self.yamlfile, WorkFast_file=self.WorkFast_file)
        AeroTwstVar, AeroBldNodes, AeroChordVarunit = TurbineMorphs.aeroelastomorph(
            xc, v_chd, xt, v_twst)
        # %%
        # Structural update: elasto twist-mass-distributuion; structural mass and stiffness update;
        newElastoBldNodes = TurbineMorphs.elastodynmorph(
            AeroTwstVar, AeroBldNodes, AeroChordVarunit)
        frequency = TurbineMorphs.bmodetrack(newElastoBldNodes)
        # Controller update
        TurbineMorphs.Ctablegen()
        BladMass, BldInertia, RotInertia = TurbineMorphs.MassCal(
            newElastoBldNodes)
        TurbineMorphs.controlupdate(frequency, vc)
        # openfast running
        super().compute(inputs, outputs)

        FASTout = FASTOutputFile(os.path.splitext(
            self.WorkFast_fileloc)[0]+".out").toDataFrame()
        FASTouts = FASTout.to_numpy()
        # Assigning the DEL
        outputs['RotThrust_max'] = FASTouts[69].max()
        outputs['RotTorq_max'] = FASTouts[70].max()
        print('################# \n'+'Response: RotTorq_max=' +
              str(FASTouts[70].max())+'\n'+'##################')
