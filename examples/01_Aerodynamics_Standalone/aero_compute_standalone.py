...
import openturbinecode.solvers.aerodynamics.aero_wrapper as OTCDaw
...
baseDir = os.path.dirname(os.path.dirname(os.path.dirname(
    os.path.abspath(__file__)))) + os.sep  # path to OTCD root

parser = argparse.ArgumentParser()
parser.add_argument("--output",
                    help="Output directory (relative to case files)",
                    type=str,
                    default="outputs")
parser.add_argument("--configuration",
                    help="WT Configuration",
                    type=str,
                    default="NREL_PhaseVI_UAE",
                    choices=["NREL_PhaseVI_UAE", "DTU_10MW"])
parser.add_argument("--fidelities",
                    help="fidelities to be included [AeroDyn, (OpenFAST,) ADflow, (turbinesFoam)]",
                    type=str,
                    default=["AeroDyn", "ADflow"],
                    nargs="+")
parser.add_argument("--path_to_case",
                    help="path where the case files are and where we will dump outputs",
                    type=str,
                    default=os.path.join(os.path.dirname(os.path.abspath(__file__)), 'case_aero_standalone'))
parser.add_argument("--hifimesh",
                    help="CFD mesh level - [0,1,2,3,4]",
                    type=int,
                    default=2)
parser.add_argument("--V",
                    help="Inflow wind speed",
                    type=float,
                    default=[7.0],
                    nargs="+")
parser.add_argument("--tsrlist",
                    help="Prescribed tip speed ratio",
                    type=float,
                    default=[5.42],
                    nargs="+")
parser.add_argument("--restart",
                    help="Name of the restart file",
                    type=str,
                    default=None)
parser.add_argument("--plotonly",
                    action='store_true',
                    help="Skip the computations (outputs must be present in folders)")
parser.add_argument("--withADres",
                    action='store_true',
                    help="Look for external AeroDyn results and plot them")
parser.add_argument("--withEllipsys",
                    action='store_true',
                    help="Look for EllipSys3D results and plot them")
...
if args.configuration == "NREL_PhaseVI_UAE":
    ...
    ADext_tsr = np.array(
        [7.58, 6.32, 5.42, 4.74, 4.21, 3.78, 3.16, 2.53, 1.90])
...
if extraFolder and args.withEllipsys:
    if args.configuration == "NREL_PhaseVI_UAE":
        # low speed shaft torque. This file contains inflow vel in the first column, and torques in the second one.
        extraFile = "LSStorque.csv"

        extraData = np.genfromtxt(os.path.join(
            path_to_case, extraFolder, extraFile), delimiter=';')
...
if 'ADflow' in args.fidelities:
    ...
    hifi_torque, hifi_thrust, hifi_cp = OTCDaw.aero_Wrapper(
        tsrlist, Vlist, pitchlist, T, rho, R0, R, Nblade, options)
...
if 'OpenFAST' in args.fidelities:
    ...
    lofi_torque, lofi_thrust, lofi_cp = OTCDaw.aero_Wrapper(
        tsrlist, Vlist, pitchlist, T, rho, R0, R, Nblade, options)
...
if 'AeroDyn' in args.fidelities:
    ...
    AD_torque, AD_thrust, AD_cp = OTCDaw.aero_Wrapper(
        tsrlist, Vlist, pitchlist, T, rho, R0, R, Nblade, options)
...
if 'turbinesFoam' in args.fidelities:
    ...
    foam_torque, foam_thrust, foam_cp = OTCDaw.aero_Wrapper(
        tsrlist, Vlist, pitchlist, T, rho, R0, R, Nblade, options)
    ...
    if MPI.COMM_WORLD.rank == 0:
        for i in range(len(ADext_vel)):  # Looping over a range of input tip speed ratios
            ...
            for j in range(len(suffixes)):
                # Caution: the naming of the files assumes that they are numbered in
                # the same order as the list of velocity you provide in ADext_vel
                outputFile = os.path.join(
                    outputDirectory + suffixes[j], f"{Tag:s}.{i+1:d}.out")

                # postprocessing output files
                thrust, torque, power, fN, fT = OTCDparser.OFparse(
                    outputFile, nodeR)

                cp, pwr, rpm, om, tip_speed = ut.WT_performance(
                    Vel, R, np.pi*R**2, rho, tsr, torque)
...
if MPI.COMM_WORLD.rank == 0:
    ...
    if os.path.exists(exp_folder):
        for i in range(len(Vlist)):  # Looping over a range of input tip speed ratios
            tsr = tsrlist[i]
            Vel = Vlist[i]
            Et[i], Eq[i], Ep[i], Ets[i], Eqs[i], Eps[i] = OTCDparser.UAEHparse(
                os.path.join(exp_folder, "uae6.z07.00.h%02.0f00000.hd1" % Vel))
            Ecp[i], pwr, rpm, om, tip_speed = ut.WT_performance(
                Vel, R, np.pi*R**2, rho, tsr, Eq[i])
            Ecps[i], pwr, rpm, om, tip_speed = ut.WT_performance(
                Vel, R, np.pi*R**2, rho, tsr, Eqs[i])
    ...
    if extraFolder and args.withEllipsys:
        ...
        plt.plot(extraX, extraCp, label=extraFolder,
                 marker="^", markersize=8, color=(0, 0.5, 0))

        # plt.plot(extraX2, extraCp2, label="ADflow", marker="+")

    if not np.isnan(sum(Eq)):
        plt.errorbar(xvals, Ecp, yerr=Ecps, color='black',
                     label='experiment', marker="D")
    # ax.set_xlim(0, -40)
    ...
    if 'ADflow' in args.fidelities:
        plt.plot(xvals, hifi_torque, label="ADflow",
                 marker="x", color=(0.5, 0, 0))
    if 'OpenFAST' in args.fidelities:
    ...
    if not np.isnan(sum(Eq)):
        plt.errorbar(xvals, Eq, yerr=Eqs, color='black',
                     label='Expe', marker="D")
    # ax.set_xlim(0, -40)
